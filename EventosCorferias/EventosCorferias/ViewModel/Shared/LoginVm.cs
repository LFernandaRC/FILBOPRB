    ...
    public class LoginVm : BaseViewModel
    {
        ...
        private async void Ingresar_Google_MtdAndroid()
        {
            ...
            try
            {
                ...
                if (loggedUser == null)
                {
                    await pageServicio.DisplayAlert(AppResources.nombreMarca, "loggedUser null", AppResources.VMaceptar);
                    return;
                }

                string urli = logicaWs.Movile_AvalidarUsuarioExistente_Mtd(loggedUser.Email, "2");
                var o = await logicaWs.ConectionPost("", urli);

                if (string.IsNullOrWhiteSpace(o))
                {
                    await pageServicio.DisplayAlert(AppResources.nombreMarca, "Para el usuario " + loggedUser.Email + "No se trajo información, intena de nuevo más tarde", AppResources.VMaceptar);
                    return;
                }

                if (o.Equals("Usuario No Existe"))
                {
                    GuardarPreferenciasUsuarioNuevo(loggedUser);
                    await Application.Current.MainPage.Navigation.PushModalAsync(new RegistroView());
                    return;
                }

                await ProcesarUsuarioExistente(o, loggedUser.Email);
                Application.Current!.MainPage = new MasterHomePage();
            }
            catch (Exception ex)
            {
                claseBase.InsertarLogs_Mtd("ERROR", ex.Message, "LoginVm", "Ingresar_Google_Mtd2", "n/a");
        ...
        private async void Ingresar_Google_MtdiOS()
        {
            ...
            {
                IsBusy = true;
                await Task.Delay(100); // Permite actualizar la UI

                GoogleUserDTO loggedUser = await _googleAuthService.AuthenticateAsync();
                if (loggedUser == null)
                {
                    await pageServicio.DisplayAlert(AppResources.nombreMarca, "loggedUser null", AppResources.VMaceptar);
                    return;
